...
export default function Home() {
  ...
  const loadMorePokemon = useCallback(async () => {
    ...
    setLoadingMore(true);
    try {
      const nextPage = Math.floor(pokemon.length / 50) + 1;
      const response = await fetch(`/api/pokemon/progressive?page=${nextPage}&limit=50`);
      
      if (response.ok) {
        const data = await response.json();
        if (data.pokemon && data.pokemon.length > 0) {
          const newPokemon = [...pokemon, ...data.pokemon];
