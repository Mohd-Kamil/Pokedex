...
export default function Home() {
  ...
  // Progressive loading of Pokemon
  useEffect(() => {
    const fetchInitialPokemon = async () => {
      try {
        console.log('Fetching initial Pokemon data...');
        const response = await fetch('/api/pokemon/progressive?page=1&limit=50');
        if (!response.ok) {
          throw new Error(`HTTP error! status: ${response.status}`);
        }
        
        const data = await response.json();
        console.log('Received initial Pokemon data:', data);
        ...
        if (data.pokemon && data.pokemon.length > 0) {
          ...
          throw new Error('No Pokemon received from API');
        }
      } catch (error) {
  ...
  // Load all remaining Pokemon at once
  const loadMorePokemon = useCallback(async () => {
    if (loadingMore || !pagination?.hasNext) return;
    
    setLoadingMore(true);
    try {
      console.log('Loading all remaining Pokemon...');
      
      // Calculate how many Pokemon we need to load
      const remainingCount = pagination.total - pokemon.length;
      const totalPages = Math.ceil(pagination.total / 50);
      const currentPage = Math.ceil(pokemon.length / 50);
      
      console.log(`Current: ${pokemon.length}, Total: ${pagination.total}, Remaining: ${remainingCount}`);
      console.log(`Current page: ${currentPage}, Total pages: ${totalPages}`);
      
      // Load all remaining Pokemon in parallel
      const remainingPages: number[] = [];
      for (let page = currentPage + 1; page <= totalPages; page++) {
        remainingPages.push(page);
      }
      
      console.log(`Loading ${remainingPages.length} remaining pages...`);
      
      // Fetch all remaining pages in parallel
      const pagePromises = remainingPages.map(async (page) => {
        const response = await fetch(`/api/pokemon/progressive?page=${page}&limit=50`);
        if (response.ok) {
          const data = await response.json();
          return data.pokemon || [];
        }
      ...
      // Combine all results
      const allNewPokemon = pageResults.flat();
      console.log(`Loaded ${allNewPokemon.length} new Pokemon`);
      
      if (allNewPokemon.length > 0) {
        const newPokemon = [...pokemon, ...allNewPokemon];
        ...
        setPagination(prev => prev ? {
          ...prev,
          hasNext: false, // No more pages to load
          page: totalPages
        } : null);
        
        console.log(`Total Pokemon loaded: ${newPokemon.length}`);
      }
  ...
  }, [loadingMore, pagination, pokemon]);
    ...
    return (
      <div className="pokedex-gray p-4 border-b-2 border-gray-700">
        ...
        {isMobile && (
          <div className="mb-4 flex justify-between items-center">
            <button
              ...
              className="pokedex-button bg-gray-800 border-2 border-gray-600 text-white hover:bg-gray-700 hover:border-gray-500 p-2 rounded-lg transition-all duration-200"
            >
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h16" />
              </svg>
            </button>
            <div className="text-white text-sm retro-font">
              <span className="text-blue-400">{filteredPokemon.length}</span> / <span className="text-green-400">{pokemon.length}</span>
                  ...
                  {/* Quick Stats Display */}
          {!isMobile && (
            <div className="mb-3 flex justify-center gap-4 text-xs text-gray-300 retro-font">
              <span>📊 TOTAL: {pagination?.total || 0}</span>
              <span>🎯 LOADED: {pokemon.length}</span>
              <span>🔍 FILTERED: {filteredPokemon.length}</span>
              {typeFilter !== 'all' && (
                <span className="text-blue-400">🎨 TYPE: {typeFilter.toUpperCase()}</span>
              )}
            </div>
          )}
        ...
        <div className={`${isMobile && !showMobileMenu ? 'hidden' : 'block'}`}>
          <div className={`${isMobile ? 'space-y-3' : 'flex flex-wrap gap-2 items-center justify-between'}`}>
            <div className={`${isMobile ? 'space-y-2' : 'flex gap-2'}`}>
              <div className="relative">
                <input
                  ...
                  className="w-full bg-gray-800 border-2 border-gray-600 text-white placeholder-gray-400 px-3 py-2 rounded-lg text-sm retro-font focus:border-blue-500 focus:outline-none transition-colors"
                  onKeyDown={(e) => {
                    if (e.key === 'Enter') {
                      e.preventDefault();
                    }
              ...
              {(searchTerm || typeFilter !== 'all' || sortBy !== 'id') && (
                <button
                  ...
                  className="pokedex-button bg-red-600 hover:bg-red-700 text-white px-3 py-2 rounded-lg text-sm retro-font transition-all duration-200"
                >
                  RESET
                </button>
              )}
              ...
              <div className="relative">
                <select 
                  ...
                  className="w-full bg-gray-800 border-2 border-gray-600 text-white px-3 py-2 rounded-lg text-sm retro-font focus:border-blue-500 focus:outline-none transition-colors appearance-none cursor-pointer"
                >
      ...
      <div className="pokedex-screen p-4 min-h-[500px] border-b-2 border-gray-600 crt-effect">
        {initialLoading ? (
          <div className="text-center py-20">
            ...
            <div className="text-white text-lg retro-font mb-2">LOADING POKEDEX...</div>
            <div className="text-gray-400 text-sm retro-font">Fetching Pokemon data from PokeAPI</div>
          </div>
        ) : (
          <>
            ...
            {/* Load All Pokemon Button */}
            {pagination?.hasNext && !loading && (
              <div className="text-center mt-6">
                <button
                  onClick={loadMorePokemon}
                  disabled={loadingMore}
                  className="pokedex-button bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-lg font-bold retro-font transition-all duration-200 disabled:opacity-50"
                >
                  {loadingMore ? (
                    <div className="flex items-center gap-2">
                ...
                <div className="mt-3 text-center">
                  <div className="text-gray-400 text-xs retro-font mb-1">
                    LOADED {pokemon.length} OF {pagination.total} POKEMON
                  </div>
                  <div className="w-full bg-gray-700 rounded-full h-2">
                    <div 
                      className="bg-blue-500 h-2 rounded-full transition-all duration-500"
                      style={{ width: `${(pokemon.length / pagination.total) * 100}%` }}
                    ></div>
                  </div>
                  <div className="text-gray-500 text-xs retro-font mt-1">
                    {pagination.total - pokemon.length} POKEMON REMAINING
                  </div>
                </div>
              </div>
            )}
            
            {/* All Pokemon Loaded Message */}
            {!pagination?.hasNext && pokemon.length > 0 && (
              <div className="text-center mt-6">
                <div className="bg-green-600 text-white px-6 py-3 rounded-lg font-bold retro-font">
                  🎉 ALL {pokemon.length} POKEMON LOADED!
                </div>
                <div className="text-gray-400 text-xs retro-font mt-2">
                  You now have access to the complete Pokemon database
                </div>
              </div>
            )}
          </>
      ...
      <div className="pokedex-screen p-4 min-h-[500px] border-b-2 border-gray-600 crt-effect">
        <div className="pokemon-card rounded-lg p-4">
          <div className="flex items-center justify-between mb-4">
            <button 
              onClick={() => {
                currentPokemonRequest.current = null;
